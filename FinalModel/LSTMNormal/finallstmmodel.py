# -*- coding: utf-8 -*-
"""FinalLSTMModel.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1c6lAvanM2RWGg1GSP936LxFN7gQL8Oz1
"""
...
tr_rating=tr_rating.astype(dtype="int32")
te_rating=te_rating.astype(dtype="int32")
...
def clean_data_pred(path,word_bank_text,word_bank_title):
    ...
    for i in range(len(data)):
        ...
        pred_rating = (data.loc[i,'RATING'])

        pred_rating=np.reshape(pred_rating,1).astype(dtype="int32")/5
        pred_vp=np.reshape(pred_vp,1)
        pred_vp=pred_vp.astype(dtype="int32")/2
        print(i)
        # print(pred_rtext.shape)
        # print(pred_rtitle.shape)
        # print(pred_rating.shape)
        preds_final = model.predict([pred_rtext,pred_rtitle,pred_rating])
        print(preds_final)
        # break
...
# clean_data_pred("Data_Pred.csv",word_bank_text,word_bank_title)
print_vp,print_rtext,print_rtitle,print_rating,print_label,clean_text,clean_title = clean_data_pred("Data_Pred.csv",word_bank_text,word_bank_title)
...
with open("Data_Pred.csv", "r" ) as cfile:
  ...
  with open("Data_Pred_final.csv", "w") as cwrite:
    ...
    for i in reader:
      writer.writerow([print_rtext,print_rtitle,print_rating,print_vp,print_label,clean_text,clean_title])
...
diff = preds.flatten() - yte
percentDiff = (yte / diff)
percentDiff = 1/percentDiff * 100
