...
tr_rtext=tr_rtext/16005
te_rtext=te_rtext/16005
tr_rtitle=tr_rtitle/16005
te_rtitle=te_rtitle/16005
...
model = Sequential()
model.add(Embedding(top_words+5, embedding_vecor_length, input_length=500))
...
embedding_vecor_length = 32
model = Sequential()
model.add(Embedding(top_words+5, embedding_vecor_length, input_length=50))
# model.add(Dropout(0.2))
...
def clean_data_pred(path,word_bank_text,word_bank_title):
    ...
    for i in range(len(data)):
        ...
        pred_rtext = np.array(x_pred_text)/16005
        pred_rating = (data.loc[i,'RATING'])/5
        pred_rtitle = np.array(x_pred_title)/16005
